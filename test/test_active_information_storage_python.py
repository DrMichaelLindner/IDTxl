...
@jpype_missing
@opencl_missing
def test_compare_Python_open_cl_estimator():
    """Compare results from OpenCl and Python estimators for AIS calculation."""
    data = Data(seed=SEED)
    ...
    settings["cmi_estimator"] = "PythonKraskovCMI"
    res_Python = network_analysis.analyse_network(settings, data, processes)
    ...
    ais_Python_2 = res_Python._single_process[2].ais
    ais_opencl_3 = res_opencl._single_process[3].ais
    ais_Python_3 = res_Python._single_process[3].ais
    print(
        "AIS for MUTE data proc 2 - opencl: {0} and Python: {1}".format(
            ais_opencl_2, ais_Python_2
        )
    )
    print(
        "AIS for MUTE data proc 3 - opencl: {0} and Python: {1}".format(
            ais_opencl_3, ais_Python_3
        )
    )
    if not (ais_opencl_2 is np.nan or ais_Python_2 is np.nan):
        assert (
            ais_opencl_2 - ais_Python_2
        ) < 0.05, "AIS results differ between OpenCl and Python estimator."
    else:
        assert (
            ais_opencl_2 is ais_Python_2
        ), "AIS results differ between OpenCl and Python estimator."
    if not (ais_opencl_3 is np.nan or ais_Python_3 is np.nan):
        assert (
            ais_opencl_3 - ais_Python_3
        ) < 0.05, "AIS results differ between OpenCl and Python estimator."
    else:
        assert (
            ais_opencl_3 is ais_Python_3
        ), "AIS results differ between OpenCl and Python estimator."
...
if __name__ == "__main__":
    ...
    test_compare_Python_open_cl_estimator()
