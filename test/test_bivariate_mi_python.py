...
def test_gauss_data():
    ...
    # Compare BivarateMI() estimate to Python estimate.
    est = PythonKraskovMI({"lag_mi": 1, "normalise": False, "noise_level": 0})
    Python_mi = est.estimate(var1=source, var2=target)
    print(
        "Estimated MI: {0:0.6f}, estimated MI using Python core estimator: "
...
def test_zero_lag():
    ...
    mi_estimator = PythonKraskovMI(settings={"normalise": False, "noise_level": 0})
    Python_mi = mi_estimator.estimate(source, target)
    omnibus_mi = results.get_single_target(1, fdr=False).omnibus_mi
    print(
        "Estimated omnibus MI: {0:0.6f}, estimated MI using Python core "
