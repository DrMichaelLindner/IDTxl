...
def test_gauss_data():
    ...
    # Compare BivarateTE() estimate to Python estimate.
    est = PythonKraskovTE(
        {
            "history_target": 1,
            "history_source": 1,
    ...
    print(
        "Estimated TE: {0:0.6f}, estimated TE using Python core estimator: "
        "{1:0.6f} (expected: ~ {2:0.6f}).".format(te, Python_cmi, expected_mi)
    )
    assert np.isclose(te, Python_cmi, atol=0.005), (
        "Estimated TE {0:0.6f} differs from Python estimate {1:0.6f} (expected: "
        "TE {2:0.6f}).".format(te, Python_cmi, expected_mi)
    )
    assert np.isclose(
