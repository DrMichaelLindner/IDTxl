...
def test_mpi_estimator_creation():
    ...
    assert isinstance(
        estimators_mpi._worker_estimator, PythonKraskovCMI
...
def test_mpi_estimation():
    ...
    # Create estimator
    jidtEstimator = PythonKraskovCMI(settings={"noise_level": 0})
    mpiEstimator = MPIEstimator("PythonKraskovCMI", dict(max_workers=MAX_WORKERS, noise_level=0))
