...
def test_gauss_data():
    ...
    # Compare BivarateMI() estimate to Python estimate. Mimick realisations used
    # internally by the algorithm.
    est = PythonKraskovMI({'lag_mi': 0, 'normalise': False, 'noise_level': 0})
    Python_mi = est.estimate(var1=source[1:-1], var2=target[2:])
    print('Estimated MI: {0:0.6f}, estimated MI using Python core estimator: '
          '{1:0.6f} (expected: {2:0.6f}).'.format(mi, Python_mi, expected_mi))
    assert np.isclose(mi, Python_mi, atol=0.005), (
        'Estimated MI {0:0.6f} differs from Python estimate {1:0.6f} (expected: '
        'MI {2:0.6f}).'.format(mi, Python_mi, expected_mi))
...
def test_zero_lag():
    ...
    mi_estimator = PythonKraskovMI(settings={'normalise': False})
    Python_mi = mi_estimator.estimate(source, target)
    omnibus_mi = results.get_single_target(1, fdr=False).omnibus_mi
    print('Estimated omnibus MI: {0:0.6f}, estimated MI using Python core '
          'estimator: {1:0.6f} (expected: {2:0.6f}).'.format(
              omnibus_mi, Python_mi, expected_mi))
    assert np.isclose(omnibus_mi, Python_mi, atol=0.005), (
        'Zero-lag omnibus MI ({0:0.6f}) differs from Python estimate '
        '({1:0.6f}).'.format(omnibus_mi, Python_mi))
