...
def test_gauss_data():
    ...
    Python_cmi = est.estimate(source=source, target=target)
    print(
        "Estimated MI: {0:0.6f}, estimated MI using Python core estimator: "
        "{1:0.6f} (expected: {2:0.6f}).".format(te, Python_cmi, expected_mi)
    )
    assert np.isclose(te, Python_cmi, atol=0.005), (
        "Estimated MI {0:0.6f} differs from Python estimate {1:0.6f} (expected: "
        "MI {2:0.6f}).".format(te, Python_cmi, expected_mi)
    )
    assert np.isclose(
